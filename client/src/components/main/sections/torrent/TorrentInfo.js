                ...
                <div key={i} className="card card-block torrent-info">
                    {torrent.openning
                        ? <div className="spinner-overlay">
                                <div className="spinner spinner-torrent"><span/><span/><span/></div>
                            </div>
                        : null}
                    <div className="row torrent-info-row">
                        ...
                        <div className="col-md-6 col-sm-12 col-xs-12 text-sm-right">
                            <div className="torrent-status">
                                ...
                                <span>
                                    ({units(torrent.status.downps)}/s)</span>
                                <img src={require('../../../../assets/icon/download.svg')} className="torrent-download-icon" alt=""/>
                            </div>
                            {torrent.open
                                ? <a className="torrent-action torrent-zip" onClick={() => this.handlezipAll(torrent.hash)}><img src={require('../../../../assets/icon/save.svg')} className="torrent-icon" alt=""/>
                                        <span>Zip</span>
                                    </a>
                                : <a className="torrent-action torrent-zip" onClick={() => this.handleOpen(torrent.hash)}><img src={require('../../../../assets/icon/play.svg')} className="torrent-icon" alt=""/>
                                    <span>Start</span>
                                </a>}
                            {torrent.open
                                ? <a className="torrent-action torrent-stop" onClick={() => this.handleStop(torrent.hash)}>
                                        <img src={require('../../../../assets/icon/stop.svg')} className="torrent-icon" alt=""/>
                                        <span>Stop</span>
                                    </a>
                                : <a className="torrent-action torrent-stop" onClick={() => this.handleRemove(torrent.hash)}>
                                    <img src={require('../../../../assets/icon/delete.svg')} className="torrent-icon" alt=""/>
                                    <span>Remove</span>
                                </a>}
                        </div>
                    </div>
                    {torrent.open && torrent.files
                                ...
                                <table className="table table-striped table-bordered ">
                                    ...
                                    <tbody>
                                        {torrent.files.map((file, i) => {
                                            return (
                                                <tr key={i}>
                                                    <td className="text-sm-right table-file">
                                                        <div className="file-name">
                                                            <div className="torrent-progress-bar" style={{
                                                                width: ((file.downloadLength / file.length) * 90) + 15 + '%'
                                                            }}></div>{file.name}</div>
                                                    </td>
                                        ...
                                        <tr>
                                            <td className="text-sm-right">{torrent.files.length}
                                                Files</td>
                                            <td>{units(torrent.length)}
                                                Total</td>
                                            <td></td>
                                        </tr>
                                    </tbody>
                        ...
                        : <p className="torrent-inactive">This torrent is inactive. Press the green power button to reactivate or the red trash can to remove it.</p>}
                </div>
            );
        });
