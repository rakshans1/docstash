...
import mega from './backends/mega';
import sftp from './backends/sftp';

let backend = null;
process.env.NODE_ENV === 'production' ? backend = mega : backend = sftp;
backend.init(secret);
...
//         "Environment variables missing for: " + backends.join(", "));
