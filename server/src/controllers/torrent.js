...
const load = (t, email, next) => {
  ...
  //loaded, now open it
  torrents.open({hash: torrent.hash }, next);
};


//on start, reopen existing torrents
setTimeout(() => {
  if (!fs.existsSync(TS_DIR)) return;
...
torrents.load = (data, next) => {
  if (data.magnet) {
    ...
    request({
      ...
      if (err) return next("Invalid Torrent Url");
      let t;
      try {
        t = parse(body);
      } catch(e) {
...
torrents.open = (data, next) => {
  ...
  if (torrent.$engine) return next("Torrent already open");
...
torrents.close = (data, next)=> {
  const torrent = findTorrent(data.hash);
  if (!torrent) return next("Torrent Missing");
  if(!torrent.$engine) return next("Torrent not open");
...
torrents.downloadFile = (data, next) => {
  ...
  if (!file) return next("File Missing");
  if(file.downloading) return next("Already Downloading");
...
torrents.cancelFile = (data, next) => {
  ...
  const file = findFile(torrent, data.path);
  if (!file) return next("File Missing");
  if(!file.downloading) return next("Not Downloading");

  const success = file.cancel();
  next(success ? null : "Failed to close file");
};
...
torrents.zipAll = (data, next) => {
  ...
  backend.upload({
    ...
    torrent.zipping = false;
    torrents.emit("update");
    if (err) return console.error("Zip upload error: " , err);
    backend.list((err, files) => {
      if (err) return console.error("Failed To list");
      torrents.emit("update", files);
    });
...
torrents.trash = (data, next) => {
  if (!data.path) return next("Path Missing");
  backend.remove(data.path, (err) => {
    if (err) return next("Failed to trash:" , err);
